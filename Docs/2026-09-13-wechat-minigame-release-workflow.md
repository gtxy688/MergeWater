...
| 广告模式（Mock ↔ 真微信） | 场景 `GameRoot` 上的 `GameBootstrapper`：`adsMode`（Mock/WeChat）、`mockAdResult`（Success/Cancel/Error）、`mockAdSeconds`（默认 1.5 秒）、`weChatRewardedAdUnitId`（`adunit-xxxxxxxx`）。**当前默认 Mock**（未开通流量主）；改成 WeChat 后若在编辑器/非小游戏运行时，会自动退回 Mock 并在 Console 告警 |

...
- [ ] **生成 Sprite 图集，并用数字确认收益**（2026-09-14 需求方「你能帮我打一下图集吗？这样能节省渲染开销吧」）：生成器 + 门禁已就位（`Assets/Scripts/Editor/SpriteAtlasGenerator.cs`、`Assets/Tests/EditMode/Presentation/SpriteAtlasTests.cs`）。步骤：① 点菜单 `MergeWater/Generate UI Sprite Atlas`（净收益：那 27 张小图不在 `Resources` 下，进图集不增包体）；② 在 Game 视图 Stats 里记录**打图集前后**的 `Batches` / `SetPass calls`（水果堆叠时一次、打开设置面板时一次）→ 用数字判断值不值；③ 星球图集（`Generate Fruit Sprite Atlas`）**暂缓**：贴图在 `Resources/` 下会被无条件打进包，图集可能让包体 +2.6MB，等量过包体再决定。**注意**：图集省的是 draw call / SetPass，**不解决 overdraw**；若数字显示瓶颈是填充率（背景 + 120 颗带透明边星球叠着画），收益会有限。

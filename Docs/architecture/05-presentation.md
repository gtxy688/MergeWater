...
- **Sprite 图集**（2026-09-14）：`Assets/Atlases/UIAtlas.spriteatlas` 收纳 `Assets/UI/Art/` 下 27 张小图（button/panel/icon/bar/toggle），并成 1 张贴图以减少 UI 的 SetPass / Batches；**满屏大图刻意排除**（`bg_star`、`Gamebg`、`bg_night`——各自只画一次，进图集只会占地方并可能顶过 2048 上限）。`Assets/Atlases/FruitAtlas.spriteatlas` 按**目录**打包 `Resources/kenney_planets/Planets`（合批收益最大：场上最多 120 颗水果，同贴图才能合批），但它**有包体风险**——`Resources` 下的贴图会被无条件打进包，图集可能让同样的像素各带一份（约 +2.6MB），因此单独一个菜单、启用前先量包体。生成入口：菜单 `MergeWater/Generate UI Sprite Atlas` / `Generate Fruit Sprite Atlas`（`SpriteAtlasGenerator`，只产图集资产、不碰场景）。打包设置统一：不旋转、Tight、padding 4、无 mipmap、不可读、`maxTextureSize 2048`（ES2 只保证 2048²）。门禁 `SpriteAtlasTests`：内容与 `Assets/UI/Art` 实际文件一致、排除项确实没进图集、尺寸 ≤ 2048、以及「新增美术忘了进图集」的漂移检查。**实测（2026-09-14，编辑器 Stats）**：开局无堆叠 `Batches 12 → 3`、`SetPass 5 → 3`；堆满星球时 `Batches 20 → 15`（该场景批次由 10 张星球贴图主导）。**星球图集不启用**（见 requirements V2.55）：`Resources/` 下贴图会被无条件打进包，约 +2.6MB 换取约 8 个 batch，不值。

...
`ReleaseDrop(x)` 由 M4/M7 在松手时调用：校验阶段与冷却 → 从队列取当前等级 → `IFieldPort.Drop` → 队列前移 → 发 `DropPerformed`。`MergeEvent` 到达时计分：`ScoreRules.ScoreFor(resultLevel, combo.Multiplier)` → 累加 → `StageProgress.Advance` → 发 `Scored`/`ComboChanged`/`MilestoneReached`；顶级（10 级）不产生 `Merged`，故不计分。

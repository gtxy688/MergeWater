...
> 相关需求：`Docs/requirements.md` 的 R1、R2、R8、R11、V2.26（R12/R13 已于 V2.58 删除）
> 验收文档：`04-aim-test.md`

## 职责边界

- 本模块负责：把指针（触摸/鼠标）输入翻译为瞄准状态；落点 x 的夹取；抛物线 + 反弹预演的采样；松手时发出投放指令（V2.58 起不再有道具瞄准模式与 `ItemTargetRequested`）；把可交互性交给 M7/M3 控制。
...
- 处理顺序：落点 x 处**先垂直下落到落点表面**（表面 y 由注入的 `IPreviewObstacle` 提供，通常是 `GameField`；无水果时即地面），随后在该表面做不超过 V2.26 的 0.8s 镜像反弹预演。
- 成功结果：返回有序点列，最低点到达落点表面（玩家能看出落点）。
- 失败与边界：重力为 0 时按 9.81 估算下落时间，返回两点直线，不死循环。
- 关键约束：**下落段不占用 0.8s 预算**。V2.26 的 0.8s 指落地后的反弹预演；若把下落也算进去，从生成高度落到地面需约 1.4s，预测线会截断在半空（实测截断在 y≈2.06），玩家看不出落点。
- 重力取值：预览使用**当前待投等级的实际重力**（`Physics2D.gravity.y × GameBalance.GetGravityScale(level)`，见 V2.28），由 M7 在每帧同步，否则预测落点会与真实下落不符。
...
| `BuildPreview(points)` | 方法 | 点列容器 | 采样点数 | 下落到落点表面 + ≤0.8s 镜像反弹（V2.26）；点数 <2 时表现层应隐藏虚线 |
| `SetObstacleProvider(provider)` | 方法 | `IPreviewObstacle` | — | 运行时由 M7 注入 `GameField`，使预览停在堆叠表面 |

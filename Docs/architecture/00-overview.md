...
- UI：uGUI + **TextMeshPro**（决策 **D16**）：文本一律用 TMP，字体为随包分发的 `ChineseUI SDF`（`Assets/Fonts/`，编辑期静态烘焙，运行时零字形生成）。原 legacy `Text` + 运行时系统字体的方案已废弃——微信小游戏（WebGL）无法访问系统字体，真机会显示方块。UI 对象全部预先存在于 `Main.unity` 里，运行时**不生成界面**；工程里也**没有界面生成器**（2026-09-13 需求方要求「以后只手动编辑 UI」后，`HudBuilder` / `SceneBuilder` / `UiTmpMigrator` 已删除，改 UI 一律在场景里改）
- 物理：Unity 2D 物理（Rigidbody2D + CircleCollider2D，CCD）
- 资源与配置：占位美术 `Assets/Resources/Placeholder/`（缺图时的兜底，运行时 `Resources.Load`）、正式水果美术 `Assets/Resources/kenney_planets/Planets/planet00..09.png`（等级 1..10 依次对应，Kenney CC0 可商用；素材包其余部分刻意放在 `Assets/Art/kenney_planets/` 不进包）、合成特效贴图 `Assets/Art/Vfx/merge_spark.png` + 材质 `MergeSpark.mat`（V2.56 行星主题火花，由场景引用决定进包）、UI 美术 `Assets/UI/Art/`（**刻意不放 `Resources/`**——`Resources` 里的资源会被无条件打进包，而 UI 图只要被场景引用就会随包分发；2026-09-13 移出后首包少 4.9MB）、数值与开关 `Assets/Config/{GameBalance,MetaSettings}.asset`、中文字体 `Assets/Fonts/{TMPCharacters.txt,ChineseUI SDF.asset}`、主场景 `Assets/Scenes/Main.unity`（**UI 的唯一来源，手工维护**）、**Sprite 图集** `Assets/Atlases/{UIAtlas,FruitAtlas}.spriteatlas`（菜单 `MergeWater/Generate UI Sprite Atlas` / `Generate Fruit Sprite Atlas`；Sprite Atlas V2 已随 `com.unity.feature.2d` 可用，**未新增依赖**；合成特效贴图刻意不进图集——粒子材质直接引用原贴图，进图集会让引用落到图集页上，行为与收益都不明确）；生成入口分别为菜单 `MergeWater/Generate Placeholder Art`、`MergeWater/Font/1·2`、`MergeWater/Generate Config Assets`——**场景与界面没有生成器**。MVP 不引入 Addressables
...
| M2 | Field 对局场地 | 水果实体的生成与销毁、2D 物理堆叠与合成解析、越线物理查询、道具对场地的直接作用（**清屏**/摇一摇；炸弹/锤子已于 V2.58 删除） | 计分与连击、状态机、广告与库存、HUD | `02-field.md` | `02-field-test.md` |
...
| M6 | Meta 局外系统 | 本地存档与迁移、道具库存与每日上限、激励视频/插屏广告适配器与频次规则、分享记录与冷却、隐私合规门、埋点 | 局内规则与物理、具体 UI 绘制 | `06-meta.md` | `06-meta-test.md` |
...
| 物理/堆叠/碰撞/合成/穿隧/抖动/CCD/限速/睡眠/警戒线检测/清屏/摇一摇/水果生成 | M2 | `Docs/architecture/02-field.md` |
...
| 存档/读档/版本迁移/最高分/道具库存/每日上限/激励视频/插屏/广告冷却/分享/隐私政策/埋点/设置开关 | M6 | `Docs/architecture/06-meta.md` |
| 启动/初始化顺序/场景装配/组合根/新手引导/两侧入口/红点/首启隐私弹窗 | M7 | `Docs/architecture/07-bootstrap.md` |
